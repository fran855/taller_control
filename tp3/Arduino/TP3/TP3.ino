...
float F[2] = {0.0224, 1.5658};
float H[2] = {0, 0};

bool primera_iteracion = true;

//float K[4] = {0.6, 0.5, 0.003, 0.002};
...
void loop() {
  ...
  float u = controlador_con_h(theta, dtheta, phi, dphi, ref_k);
...
float controlador_con_h(float theta, float dtheta, float phi, float dphi, float ref[2]){
  float u =  K[0] * theta + K[1] * dtheta + K[2] * (phi - phi_abs) + K[3] * dphi + H[0] * (ref[0] + theta_ref_1 - theta - theta_1) + H[1] * (ref[1] + phi_ref_1 - phi - phi_1);
  // se cancelan los phi_abs
  theta_ref_1 = ref[0];
  phi_ref_1 = ref[1];
  return u;
}
